...
def spherical_Lloyd(radius, num_cells, dimension=3, fixed='center', approximation='monte-carlo',
    ...
    for iter in range(max_iter):
        ...
        if verbose:
            logging.log('iter {:5d} / max move = {:f}'.format(iter, np.max(np.linalg.norm(moves, axis=1))))
            if warning:
                logging.warning('{t least one point has no cell')
...
def kernel_point_optimization_debug(radius, num_points, num_kernels=1, dimension=3,
    ...
    while step < 10000:
        ...
        if verbose:
            logging.log('step {:5d} / max grad = {:f}'.format(step, np.max(gradients_norms[:, 3:])))
