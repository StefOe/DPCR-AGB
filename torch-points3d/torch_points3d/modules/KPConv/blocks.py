...
@torch.jit.script
def gather(x: torch.Tensor, idx: torch.Tensor, method: int = 2):
    """
...
@torch.jit.script
def radius_gaussian(sq_r: torch.Tensor, sig: float, eps: float = 1e-9):
    """
...
@torch.jit.script
def closest_pool(x: torch.Tensor, inds: torch.Tensor):
    """
...
@torch.jit.script
def max_pool(x: torch.Tensor, inds: torch.Tensor):
    """
...
@torch.jit.script
def global_average(x: torch.Tensor, batch_lengths: torch.Tensor):
    """
...
@torch.jit.script
def global_sum(x: torch.Tensor, batch_lengths: torch.Tensor):
    """
    Block performing a global average over batch pooling
    :param x: [N, D] input features
    :param batch_lengths: [B] list of batch lengths
...
def block_decider(block_name,
                  ...
                  config):
    if block_name == 'unary':
        return UnaryBlock(in_dim, out_dim, act_fn, config.use_batch_norm, config.batch_norm_momentum)
        ...
        return SimpleBlock(block_name, in_dim, out_dim, radius, layer_ind, act_fn, config)
        ...
        return ResnetBottleneckBlock(block_name, in_dim, out_dim, radius, layer_ind, act_fn, config)
    ...
    elif block_name == 'global_sum':
        return GlobalSumBlock()
...
class UnaryBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, act_fn, use_bn, bn_momentum, no_relu=False):
        """
        Initialize a standard unary block with its ReLU and BatchNorm.
        :param in_dim: dimension input features
        :param out_dim: dimension input features
        :param act_fn: activation function (nn.Module) to initialize
        :param use_bn: boolean indicating if we use Batch Norm
        :param bn_momentum: Batch norm momentum
        """
...
class SimpleBlock(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, act_fn, config):
        """
    ...
    def forward(self, x, batch):
        ...
        return self.act(self.batch_norm(x))
...
class ResnetBottleneckBlock(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, act_fn, config):
        ...
        if in_dim != out_dim // 4:
            self.unary1 = UnaryBlock(in_dim, out_dim // 4, act_fn, self.use_bn, self.bn_momentum)
        else:
        ...
        self.unary2 = UnaryBlock(out_dim // 4, out_dim, act_fn, self.use_bn, self.bn_momentum, no_relu=True)

        # Shortcut optional mpl
        if in_dim != out_dim:
            self.unary_shortcut = UnaryBlock(in_dim, out_dim, act_fn, self.use_bn, self.bn_momentum, no_relu=True)
        else:
    ...
    def forward(self, features, batch):
        ...
        # Convolution
        x = self.KPConv(q_pts, s_pts, neighb_inds, x)
        x = self.act(self.batch_norm_conv(x))
...
class GlobalSumBlock(nn.Module):

    def __init__(self):
        """
        Initialize a global average block with its ReLU and BatchNorm.
        """
        super(GlobalSumBlock, self).__init__()
        return

    def forward(self, x, batch):
        return global_sum(x, batch.lengths[-1])
