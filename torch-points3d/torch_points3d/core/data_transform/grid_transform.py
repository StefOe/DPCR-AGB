...
_INTEGER_LABEL_KEYS = ["y", "y_cls", "instance_labels"]
...
def group_data(data, cluster=None, unique_pos_indices=None, mode="last", skip_keys=[]):
    ...
    The option ``mode`` controls how data gets aggregated within each cluster.
    ...
    mode : str
        ...
        ``last`` selects the last point falling in a voxel as the represented, ``mean`` takes the average.
    skip_keys: list
...
class GridSampling3D:
    ...
    def _process(self, data):
        ...
        if "batch" not in data:
            ...
            cluster = voxel_grid(pos=coords, size=1, batch=data.batch)
        ...
        if self._apply_distorsion:
            if random.random() < 0.95:
                for i in range(len(self._granularity)):
                    data.pos = ElasticDistortion.elastic_distortion(data.pos, self._granularity[i],
                                                                    self._magnitude[i], )
        return data
