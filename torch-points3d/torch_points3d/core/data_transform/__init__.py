...
if L_intersection_names:
    if L_intersection_cls:
        raise Exception(
            "It seems that you are overriding a transform from pytorch gemetric, \
                this is forbidden, please rename your classes {} from {}".format(
...
class RandomParamTransform(object):
    ...
    transform_options: Omegaconf Dict
        contains the name of a variables as a key and min max type as value to specify the range of the parameters and
        the type of the parameters or it contains the value "value" to specify a variables (see Example above)
